...
v1.2の変更点:
  ...
  ・probe、trim、mixコマンドと、機械処理向けのJSON出力に対応
...
「JSONを開く」と「JSON保存」では、タイムラインの内容をプロジェクトとして
読み書きできます。クリップを選んで「詳細設定」を開くと、音声内の開始・終了、
音量、フェード、ループ、ミュートを変更できます。


■ CodexやPowerShellから画面を開かずに使う

AudioAtelier.exeは、引数を付けずに起動すると通常の画面を開きます。
次のコマンドをPowerShellやコマンドプロンプトから実行した場合は、画面を
開かず、処理が終わるまで待ってから結果を返します。
...
指定した秒数の音声を書き出す:
  ...
  .\AudioAtelier.exe trim --input "input.mp4" --start 12.5 --end 18.2 `
    --output "voice.wav" --json

JSONプロジェクトを合成する:

  .\AudioAtelier.exe mix --project "project.json" `
    --output "final_mix.wav" --json

正常に完了すると終了コード0、失敗すると0以外を返します。--jsonを付けると
標準出力へ結果JSON、失敗時は標準エラー出力へエラーJSONを出します。結果には
入出力パス、処理時間、生成ファイル、FFmpegの終了コードが含まれます。

--dry-runを付けるとFFmpegを実行せず、実行予定のコマンドを確認できます。
mixではfilter_complexも返します。既存ファイルは初期状態では上書きしません。
上書きする場合だけ--overwriteを付けてください。


■ JSONプロジェクトを作る

次の内容をUTF-8のJSONファイルとして保存すると、GUIの「JSONを開く」と
CLIのmixコマンドの両方で使用できます。
...
pathは、JSONファイルからの相対パスまたは絶対パスで指定します。laneは1から
始まります。trim_inとtrim_outは元の音声内の秒数、startはタイムライン上の
開始秒です。ループだけのプロジェクトでは、全体の長さをdurationで指定します。

loudness_lufsをnullにするかmasterから省略した場合は、従来のサンプルピーク
リミッターを使用します。数値を指定した場合は、FFmpegのloudnormフィルターで
ラウドネスとTrue Peakの上限を調整します。


■ 音声の境界ノイズと音量超過を自動で抑えます

動画から音声を書き出すときは、切り出した範囲の先頭と末尾へ約10msの
短いフェードイン・フェードアウトを自動で適用します。合成時も各クリップの
両端へ同じ処理を行い、波形の途中で切れた際に発生しやすい「ブチッ」という
クリックノイズを抑えます。

10msより短い音声では、フェードが音声の半分を超えないよう自動で短縮します。
初期値ではノイズ対策として自動適用されます。合成クリップのフェード時間は、
「詳細設定」またはJSONプロジェクトから変更でき、0秒にすると無効になります。

複数の音声を重ねた合成結果には、合成処理中のサンプルピークが0dBFSを
超えないようリミッターを適用します。上限は0.95（約-0.45dBFS）で、
自動音量補正によって0dBFSまで持ち上げない設定です。
...
本ソフトは個人制作の無料ツールです。大切なファイルは、あらかじめ
バックアップを取ってからご利用ください。
