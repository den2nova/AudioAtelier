...
v1.3.0の変更点:
  ...
  ・JSONプロジェクトのgain_dbとCLIの互換性は従来どおり維持
...
v1.2.2の変更点:
  ...
  ・画面の試聴・書き出しとCLIのtrimを、音声サンプル単位で切り出す方式へ変更
...
Audio Atelierのソースコードは、次のGitHubリポジトリで公開しています。
中身を確認したい場合や、ソースから実行・ビルドしたい場合に参照できます。

  https://github.com/den2nova/AudioAtelier
...
クリップの音量調整では、0 dBが元の音量（100%）です。マイナスにすると小さく、
プラスにすると大きくなります。目安として、-6 dBは約50%、-12 dBは約25%、
+3 dBは約141%です。スライダーやプリセットを使えば、dBを覚えていなくても
調整できます。

ここで指定するdBは、クリップへ加える音量の差です。合成結果のピーク上限を示す
dBFSとは意味が異なります。JSONやCLIでは、従来どおりgain_dbへdB値を指定します。
...
開始秒です。gain_dbは0.0で元の音量、負の値で小さく、正の値で大きくなります。
ループだけのプロジェクトでは、全体の長さをdurationで指定します。
...
試聴用ファイルは再生後に自動削除されます。異常終了などで残った場合も、
次回起動時に自動で整理されます。
