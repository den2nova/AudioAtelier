...
- クリップ詳細の音量調整に「小さい↔大きい」のスライダーを追加
- 音量を100%換算で併記し、dBの意味が分かる説明とプリセットを追加
- タイムライン上の音量表示を、直感的なパーセント表記へ変更
- JSONプロジェクトの`gain_db`とCLIの互換性は従来どおり維持

### v1.2.3

- ごく短い範囲でも確実に試聴できるよう、選択音の末尾へ再生用の無音を追加
- 合成試聴の一時ファイルを再生後・停止時に自動削除
- 異常終了などで残った試聴用ファイルを、次回起動時に自動整理

### v1.2.2

- ごく短い範囲の試聴で、選択範囲外の音が混ざる場合がある問題を修正
- 画面の試聴・書き出しとCLIの`trim`を、音声サンプル単位で切り出す方式へ変更
...
すぐに使えるビルド済みWindows版は、右側の[Releases](../../releases)から`AudioAtelier-v1.3.0-Windows.zip`をダウンロードしてください。配布ZIPには`AudioAtelier.exe`と詳しい`README.txt`が入っています。
